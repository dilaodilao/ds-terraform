...
inputs = {
  ...
  private_subnet_names = [ for i in range(length(local.azs)) : "${upper(local.name)} -- Private --- ${local.azs[i]}" ]

  public_subnets  = [for i in range(length(local.azs)) : cidrsubnet(local.cidr_block, 8, i+100)]
  public_subnet_names = [ for i in range(length(local.azs)) : "${upper(local.name)} -- Public --- ${local.azs[i]}" ]

  database_subnets = [for i in range(length(local.azs)) : cidrsubnet(local.cidr_block, 8, i+200)]
  database_subnet_names = [ for i in range(length(local.azs)) : "${upper(local.name)} -- Database --- ${local.azs[i]}" ]
  database_subnet_group_name = "${upper(local.name)}-vpc"

  elasticache_subnets = [for i in range(length(local.azs)) : cidrsubnet(local.cidr_block, 8, i+225)]
  elasticache_subnet_names = [ for i in range(length(local.azs)) : "${upper(local.name)} -- Elasticache --- ${local.azs[i]}" ]
  elasticache_subnet_group_name = "${local.name}-vpc"
