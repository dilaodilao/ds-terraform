...
inputs = {
  ...
  name = "${upper(include.root.locals.def.env)}---${basename(get_terragrunt_dir())}"
  ...
  default_network_acl_name   = basename(get_terragrunt_dir())
  ...
  default_network_acl_ingress = [
    ...
    },
    {
      ...
      to_port    = 0
      protocol   = "-1"
      cidr_block = include.root.locals.def.cidr[include.root.locals.def.env][basename(get_terragrunt_dir())]
    },
    {
      rule_no    = 20200
      action     = "allow"
